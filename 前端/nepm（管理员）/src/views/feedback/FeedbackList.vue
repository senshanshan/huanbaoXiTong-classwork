<template>
  <section class="page">
      ...
      <el-form :inline="true" :model="query" size="small">
        ...
        <el-form-item label="预估等级">
          <el-select v-model="query.estimatedGrade" clearable placeholder="全部" style="width: 120px">
            <el-option v-for="item in aqiOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="反馈日期">
          <el-date-picker v-model="query.afDate" type="date" value-format="YYYY-MM-DD" placeholder="选择日期" style="width: 150px" />
        </el-form-item>
    ...
    <el-table :data="rows" border stripe v-loading="loading" :cell-style="{ padding: '6px' }" empty-text="无数据">
      ...
      <el-table-column label="预估等级" min-width="150">
      ...
      <el-table-column label="操作" width="130" fixed="right">
        <template #default="{ row }">
          ...
          <el-button link type="success" size="small" :disabled="row.state === 2" @click="goAssign(row.afId)">指派</el-button>
        </template>
      </el-table-column>
    </el-table>
...
const stateType = (state) => (state === 2 ? "success" : state === 1 ? "warning" : "info");
